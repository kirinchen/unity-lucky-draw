...
namespace bzk.tool {
    public class LuckyDraw<T> {
        ...
        public T draw(bool drawedRemove = false) {
            T ans = getDrawKey();
            if (drawedRemove) {
                int count = samples[ans] - 1;
                samples.Remove(ans);
                if (count > 0) {
                    samples.Add(ans, count);
                }
            }
            return ans;
        }

        private T getDrawKey() {
        ...
        public static int test() {
            ...
            LuckyDraw<TE> ld = new LuckyDraw<TE>();
            int a = 0, b = 0, c = 0;
            ld.add(TE.a, 2);
            ld.add(TE.b, 2);
            ld.add(TE.c, 2);
